...
void YsfxProcessor::getStateInformation(juce::MemoryBlock &destData)
{
    ...
    if (state) {
        ...
        juce::Array<juce::var> sliders;
        sliders.addArray(state->sliders, ysfx_max_sliders);
        stateTree.setProperty("sliders", sliders, nullptr);

        stateTree.setProperty("data", juce::Base64::toBase64(state->data, state->size), nullptr);
...
void YsfxProcessor::setStateInformation(const void *data, int sizeInBytes)
{
    ...
    if (stateTree != juce::ValueTree{}) {
        ysfx_state_t state{};
        juce::Array<ysfx_real> sliders;
        juce::MemoryBlock dataBlock;

        {
            const juce::var &slidersProperty = stateTree.getProperty("sliders");
            if (slidersProperty.isArray())
                sliders.addArray(*slidersProperty.getArray());
            sliders.resize(ysfx_max_sliders);
        }
        {
