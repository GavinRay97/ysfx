...
#define EEL_STRING_MUTEXLOCK_SCOPE std::lock_guard<std::mutex> lock{((ysfx_t *)(opaque))->string_mutex};
#define EEL_STRING_MAXUSERSTRING_LENGTH_HINT ysfx_string_max_length

static std::mutex atomic_mutex;
#define EEL_ATOMIC_SET_SCOPE(opaque) std::mutex *mutex = ((opaque) ? &((ysfx_t *)(opaque))->atomic_mutex : &atomic_mutex);
